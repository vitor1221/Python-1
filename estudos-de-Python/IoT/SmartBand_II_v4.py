...
class SmartBand:
    ...
    def __init__(self, server_ip="127.0.0.1", server_port=5000):
        ...
        """
        self.sensors = {
            "Temperature Sensor (LM35)": {"min": 28, "max": 46, "unit": "°C", "std_dev": 2.5},
            "Heart Rate (HR) Sensor (GE Healthcare MAC 5500 HD)": {"min": 50, "max": 100, "unit": "BPM", "std_dev": 5.0},
            "Accelerometer (MPU-6050)": {"min": -16, "max": 16, "unit": "g", "std_dev": 2.5},
            "Gyroscope (MPU-6050)": {"min": 100, "max": 200, "unit": "°/s", "std_dev": 10.0},
            "SpO2 Sensor (Texas Instruments AFE4400)": {"min": 85, "max": 100, "unit": "%", "std_dev": 4.0},
            "Electrodermal Activity (EDA) Sensor (ADS1299)": {"min": -0.5, "max": 2.5, "unit": "mV", "std_dev": 0.5},
            "Ambient Light Sensor (APDS-9301)": {"min": 0, "max": 50000, "unit": "lux", "std_dev": 10000},
            "Glucose Sensor (Dexcom G6)": {"min": 0.002, "max": 0.06, "unit": "%", "std_dev": 0.012}
        }
    ...
    def generate_sensor_data(self):
        """
        Gera dados aleatórios para os sensores configurados usando desvio padrão.
        :return: Um dicionário com os nomes dos sensores e seus valores gerados.
        """
        sensor_data = {}
        
        for sensor_name, sensor_info in self.sensors.items():
            # Usando random.gauss para incorporar o desvio padrão
            mean = (sensor_info["min"] + sensor_info["max"]) / 2
            value = random.gauss(mean, sensor_info["std_dev"])

            # Garantindo que o valor esteja dentro dos limites [min, max]
            value = max(sensor_info["min"], min(sensor_info["max"], value))
            sensor_data[sensor_name] = round(value, 3)  # Para glicose, precisão de 3 casas
